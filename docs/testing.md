...
| 63 | Redundancy | The live API proves per-UPS nominal-watt override/global inheritance beating NUT's reported `ups.realpower.nominal` (with exactly one above-rating warning), redundancy member + aggregate energy/failover-load telemetry, the exact group shutdown plan, and terminal phase/remote progress after real quorum loss |
