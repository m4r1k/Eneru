...
| Area | Coverage |
|------|----------|
| Config loading and validation | YAML parsing, defaults, enum validation, multi-UPS inheritance (including per-UPS energy tariff/nominal-watt overrides, explicit-null clearing, and global-only key rejection), local ownership, loopback delegation config shape, redundancy rules |
...
| Redundancy runtime | Quorum evaluation, advisory triggers, connection-grace handling, idempotent group execution, aggregate member telemetry, exact group plans, and live phase/remote progress snapshots |
...
- `tests/test_dashboard.py` covers fleet summary rendering, explicit and
  ...
  Control behavior, shutdown progress surfaces, stale Control render rejection,
...
| 63 | Redundancy | The live API proves per-UPS nominal-watt override/global inheritance, redundancy member + aggregate energy/failover-load telemetry, the exact group shutdown plan, and terminal phase/remote progress after real quorum loss |
