...
| Single-UPS `is_local` | `tests/test_single_ups_is_local.py`: a one-entry `ups:` list with an explicit `is_local: false` never powers the host off (dry-run and real mode, the T1-T4/FSD trigger path, the FAILSAFE connection-loss path, T5 monitor-only), while the mapping form, `is_local: true` and an omitted key still do; runtime vs shutdown plan vs role/action/ON_BATTERY hint vs the TUI state-file role, the omitted-key warning (validate, startup log, `config check`), `eneru validate` tree and tag, root-requirement and `/ready` capability reasons, the `config check` order tree/power-loss preview, hot-reload restart classification, and the multi-UPS ERROR finding when every entry says `is_local: false` under `trigger_on: any` |
...
| What-happens-next model | `tests/test_outlook.py`: every trigger's state/margin/ETA and the closest one, parity with `_handle_on_battery` at and around each threshold (T1-T5, stabilization, grace) plus proof that computing the outlook leaves the decision unchanged, the per-UPS role and action (local, remote-only, monitoring-only, redundancy member, delegated loopback, coordinator handoff), freshness and stale limits, the state-file keys and the progress sidecar, redundancy failing members and quorum outlook (also from state files for the TUI), the shared status vocabulary and severity, per-tier event selection, the capped battery-replacement estimate, and the API/status wiring with its error fallbacks. Also: a one-entry `ups:` list without `is_local` planned and dry-run as a host poweroff, the FAILSAFE failed-poll countdown driven through the real `_main_loop` (`arming` from the first failed poll until the poll that fires), negative drain rates, attributed self-tests, cold-start deferral, effective member health, a member's amber "group decides" summary, and a late progress write never overwriting a finished run |
...
| Config editor and checker | `tests/test_config_check.py` (static findings, section classification, NUT/SSH/local probes with mocks, the remote check script executed locally, sudo semantics, power-loss plan, report rendering, `eneru config` CLI), `tests/test_config_doc.py` (byte-identical ruamel round trip for every example and E2E config, comment placement for new keys/sections, legacy-to-multi-UPS conversion, atomic 0600 saves with `.bak`), `tests/test_config_catalog.py` (drift guard: every loader dataclass field has an explained catalog entry with the same default), `tests/test_config_tui.py` (every page, row, prompt and key through the model, curses rendering with a fake window, add/discard/delete of list items, the dry-run confirm, changed-value marks, `/` search, per-UPS values matching the loader for every overridable key, the legacy `parallel` conversion keeping identical phases, and the shutdown-order page). Also: check/runtime sudo-prefix parity, builtins and compounds flagged under sudo, `static_findings` wiring (loopback contract, privilege, delegate synthesis), probe `timeout` wrapper and marker-only parsing, every numeric catalog bound accepted by the loader, an all-defaults document validating clean, save refusing a symlink swapped in while or after load, MQTT broker credentials masked, `nan`/`inf` and padded text rejected or trimmed in the editor, per-UPS-only triggers hidden under redundancy groups, and duplicate YAML keys warned by `config check` (recursive anchors included), and the `config check` shutdown-order tree matching the runtime phases (legacy `parallel`, loopback bracket, redundancy groups, a coordinator that keeps the host on) |
...
- `tests/test_dashboard_outlook.py` covers the role-aware banner and tab
  ...
  daemons, every banner branch's text/severity/rank (connection lost, stale,
  quorum lost, at risk, redundancy members that yield to the group), the
  client-side data-age and staleness math, group names on `collect_status`
  fleet rows, the bounded replacement estimate, redundancy-group outlook, stale
  remote checks (strip, icon and badge all neutral), CSP-safe/newest-first/
  pulse-only-when-blinking guards, the amber `arming` FAILSAFE countdown
  (next-trigger line, banner rank), the connection error announced once per
  change, and control-action confirmations on the status line.
...
The numbered E2E tests are defined in `tests/e2e/groups/*.sh`. There are 71 numbered tests, two redundancy runtime regression cases, plus one CLI completion smoke check. Every group script must end with its `=== Group '<name>' completed successfully ===` line and have a matrix entry in `.github/workflows/e2e.yml`; a workflow step checks both.
...
| 43 | UPS Single | `/health`, `/ready`, `/metrics`, `/api/v1`, and JSON 404 discovery respond; a real NUT low-battery event publishes the exact per-UPS plan plus terminal phase/remote progress; `/api/v1/ups` carries the next-trigger outlook, the exact role and action, freshness and status vocabulary, and after the low-battery event `nextTrigger` is the trigger the daemon logged as fired; the state file carries `EPOCH`, and the progress sidecar matches the API |
...
| 66 | CLI | `eneru config` TUI driven through a pty (`tests/e2e/config-tui-driver.py`): an in-place edit changes one line, keeps the comment and writes `.bak`, and the captured screen shows the dry-run confirm on both the toggle and the save; a new file is 0600, explained, validates, and passes the live NUT check |
...
| 68 | Loopback | `docker exec -it <ctr> eneru config` saves a writable config bind mount in place (same host inode, operator comment kept), keeps the `.bak` 0600 in the state volume, the dry-run confirm appears on the captured screen, the container hot-reloads on SIGHUP, and a `:ro` mount opens as `[read-only]` and stays untouched |
| 69 | CLI | A real remote shutdown with split `ssh_options` (`-i KEY -p PORT`) runs both the pre-shutdown step and the final shutdown command on the target within 60 s, and `config check` reaches it with the same options |
...
| 71 | UPS Single | A one-entry `ups:` list with an explicit `is_local: false` and one remote server, under low battery: the remote shutdown runs and the host poweroff (`Would execute: shutdown -h now`) does not; with `is_local` omitted the startup log and `eneru validate` warn and the host poweroff still runs |
